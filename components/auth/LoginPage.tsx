...
const LoginPage: React.FC<LoginPageProps> = ({ onNavigateToSignup, onNavigateToLanding }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // On success, App.tsx will detect session change and rerender.
    } catch (err: any) {
  ...
  return (
    <AuthLayout title="Welcome Back, Adventurer" onNavigateToLanding={onNavigateToLanding}>
      ...
      <div className="mt-6 text-center text-sm text-muted-foreground">
        ...
        <button onClick={onNavigateToSignup} className="underline text-primary hover:text-primary/80 font-semibold">
          Sign Up
        </button>
      </div>
    </AuthLayout>
